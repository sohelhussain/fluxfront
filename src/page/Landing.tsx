...
function Landing() {
  ...
  const handleIdentify = async () => {
    ...
    try {
      const response = await axios.post("https://fluxbakend.onrender.com/identify", {
        email: email,
        phone: phone,
      });
    ...
    <div className="w-full h-screen relative bg-gradient-to-r from-purple-500 to-pink-500 flex justify-center items-center">
      <div className="w-96 bg-clip-padding p-8 backdrop-filter backdrop-blur-3xl bg-white/10 rounded-md">
        ...
        <button
          ...
          disabled={loading} 
          className={`w-full h-12 px-4 py-2 text-sm text-white rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
            loading ? "bg-gray-400 cursor-not-allowed" : "bg-blue-500 hover:bg-blue-600"
          }`}
        >
          {loading ? "Identifying..." : "Identify Response"} 
        </button>

        {loading && (
          <div className="absolute inset-0 flex justify-center items-center bg-white/10 rounded-md">
            <svg
              aria-hidden="true"
